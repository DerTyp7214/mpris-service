...
test("creating a player exports the playlists interfaces on the bus", async () => {
  let dbusObj = await bus.getProxyObject(
  ...
  expect(names).toEqual(
    expect.arrayContaining(["org.mpris.MediaPlayer2.playliststest"]),
  );
...
test("default state of the playlists interface", async () => {
  ...
  expect(activePlaylist).toEqual(
    new Variant("(b(oss))", [false, ["/", "", ""]]),
  );

  let playlists = await playlistsIface.GetPlaylists(
    0,
    1,
    "Alphabetical",
    false,
...
test("setting a playlist on the player works", async () => {
  ...
  expect(propsCb).toHaveBeenCalledWith(
    PLAYLISTS_IFACE,
    {
  ...
  // all userdefined
  let dbusPlaylists = await playlistsIface.GetPlaylists(
    0,
    99,
    "UserDefined",
    false,
  ...
  expect(player.playlists.slice(1, 3).map(playlistToDbus)).toEqual(
    dbusPlaylists,
  );
  ...
  // all alphabetical
  dbusPlaylists = await playlistsIface.GetPlaylists(
    0,
    99,
    "Alphabetical",
    false,
